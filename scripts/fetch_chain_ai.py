#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""从 Hermes Agent 会话库导出最新的「产业链涨价」分析 -> 覆盖 chain.js。

Hermes 按 agent/chain-radar.md 的 prompt 跑产业链涨价分析(AI 算力主线 + 全市场),
最终输出一个 JSON 代码块。本脚本从对应 session 提取该 JSON,
写入 chain.js(window.CHAIN)。

本地运行: python3 scripts/fetch_chain_ai.py
需 hermes 命令行可用。
"""
import json
import os
...
OUT = os.path.join(PROJ, "chain.js")
...
def extract_json_block(content):
    ...
    for m in re.finditer(r"```\s*(\{.*?\})\s*```", content, re.S):
        try:
            ...
            continue
    # 最后兜底:找裸 JSON(window.CHAIN = {...};)
    m = re.search(r"window\.CHAIN\s*=\s*(\{.*?\})\s*;?", content, re.S)
    if m:
        try:
            return json.loads(m.group(1))
        except Exception:
...
def main():
    print("查找产业链涨价 Hermes session...", flush=True)
    sessions = list_sessions(limit=100)
    if not sessions:
        print("⚠ 未找到任何 Hermes session,保留旧 chain.js 不更新。请先运行 agent/chain-radar.md 任务。", flush=True)
        return
    # 匹配产业链涨价相关 session(标题含"产业链"/"涨价"/"chain"/"AI产业链"等)
    keywords = ["产业链涨价", "产业链", "涨价雷达", "chain-radar", "AI产业链", "供需", "卡脖子", "涨价"]
    matched = []
    for s in sessions:
        t = s.get("title", "")
    ...
    if not found_json:
        print("⚠ 未在候选 session 中找到含 directions 的产业链分析 JSON,保留旧 chain.js 不更新。", flush=True)
        print("  请确认 agent/chain-radar.md 任务已运行且输出了 JSON 代码块。", flush=True)
        return

    # 写回 chain.js
    header = (
        "/* 产业链涨价数据：供需紧张/涨价方向 + 卡脖子 + 产业链逻辑\n"
        f" * 由 Hermes Agent 按 agent/chain-radar.md 分析生成\n"
    ...
    content = header + "window.CHAIN = " + json.dumps(found_json, ensure_ascii=False, indent=2) + ";\n"
    tmp = OUT + ".tmp"
    open(tmp, "w", encoding="utf-8").write(content)
    os.replace(tmp, OUT)
    print(f"✅ 已从 session「{used_title}」导出产业链涨价 -> chain.js", flush=True)
