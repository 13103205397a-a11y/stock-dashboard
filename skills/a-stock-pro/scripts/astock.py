"""a-stock-pro: A股全栈数据工具包 (V3.4.0 进化版)

基于 simonlin1212/a-stock-data V3.3.0 进化而来,主要改进:
  - 兼容 Python 3.9+ (使用 from __future__ import annotations)
  - 修复 dragon_tiger_board 空数据时 buy_data 未定义崩溃
  - 修复 lockup_expiry 日期格式只支持 YYYY-MM-DD 的问题,现兼容 YYYYMMDD
  - 修复 ths_hot_reason 列名映射在字段缺失时崩溃
  - 函数定义与示例代码彻底分离,可直接 import 无副作用
  - 全部字段名标准化,增加中文别名便于 AI/人理解
"""
...
def _normalize_date(s: str) -> str:
    """日期格式归一化: 支持 'YYYYMMDD' / 'YYYY-MM-DD' / 'YYYY/MM/DD' / 'YYYY.MM.DD' /
    'YYYY年M月D日' 等常见写法 → 统一输出 'YYYY-MM-DD'。
    无法识别时抛 ValueError(明确报错,不让静默返回错数据)。"""
    s = str(s).strip()
    if not s:
        raise ValueError("日期为空,请传入 'YYYY-MM-DD' 或 'YYYYMMDD' 等格式")
    # 去掉中文"年月日"分隔 → 标准化
    ...
    for fmt in ("%Y-%m-%d", "%Y%m%d"):
        try:
            ...
            continue
    raise ValueError(f"无法识别日期格式 '{s}',请用 'YYYY-MM-DD' 或 'YYYYMMDD'")
...
def ths_hot_reason(date: str = None) -> pd.DataFrame:
    ...
    # 字段重命名（中文友好）—— 只映射实际存在的列,避免 KeyError
    rename_map = {
...
def dragon_tiger_board(code: str, trade_date: str, look_back: int = 30) -> dict:
    """
    龙虎榜数据聚合。
    trade_date: 支持 'YYYY-MM-DD' / 'YYYYMMDD' 等格式(经 _normalize_date 归一化)
    look_back: 回看天数
    ...
    # 2. 最近上榜的买卖席位
    seats = {"buy": [], "sell": []}
    buy_data, sell_data = [], []   # 预初始化,避免 records 为空时下方引用未定义
    if records:
...
def lockup_expiry(code: str, trade_date: str, forward_days: int = 90) -> dict:
    """
    限售解禁日历。
    trade_date: 支持 'YYYY-MM-DD' 或 'YYYYMMDD' 两种格式
    返回: {history: [...], upcoming: [...]}
    """
    # 日期格式归一化:兼容 YYYYMMDD / YYYY-MM-DD
    trade_date = _normalize_date(trade_date)
    # 1. 历史解禁记录
...
def market_top(metric: str = "chgPct", n: int = 50, desc: bool = True) -> list[dict]:
    """全市场 A 股个股排名榜（东财 clist 接口,免 key,海外可跑）。

    metric: 排序指标,支持:
        chgPct(涨跌幅) / turnover(换手率) / netInflow(主力净流入元)
        amplitude(振幅) / volumeRatio(量比) / amount(成交额)
    n:      取前 N 条(1~200)
    desc:   True 降序(取最高),False 升序(取最低,用于跌幅榜/资金流出榜)
    返回: [{rank, code, name, price, chgPct, turnover, netInflow, amplitude,
            volumeRatio, amount, industry, mcap_yi}]
    """
    # 指标 → 东财字段映射 + 排序字段
    metric_map = {
        "chgPct":      ("f3",  "f3"),
    ...
    if metric not in metric_map:
        raise ValueError(f"不支持的 metric '{metric}',可选: {list(metric_map)}")
    sort_field, _ = metric_map[metric]
    n = max(1, min(int(n), 200))
    # fs: 沪深 A 股全部(沪市主板 m:0+t:6, 沪市科创板 m:1+t:23, 深市主板 m:0+t:80, 深市创业板 m:1+t:2)
    # fid/fcd: 排序字段; po: 1降序 0升序
    url = "https://push2.eastmoney.com/api/qt/clist/get"
    params = {
        "pn": "1", "pz": str(n), "po": "1" if desc else "0", "np": "1",
        "fltt": "2", "invt": "2",
        "fs": "m:0+t:6,m:0+t:80,m:1+t:2,m:1+t:23",
        "fields": f"f2,f3,f6,f7,f8,f10,f12,f14,f62,f100,f20",
        "fid": sort_field, "fcd": sort_field,
    }
    headers = {"User-Agent": UA, "Referer": "https://quote.eastmoney.com/"}
    try:
        r = em_get(url, params=params, headers=headers, timeout=15)
        ...
        print(f"[WARN] market_top({metric}) 请求失败: {e}")
        return []
    items = (d.get("data") or {}).get("diff", []) or []
    out = []
    for i, it in enumerate(items):
        # f20=总市值(元), 转成亿; f62=主力净流入(元)
        mcap = it.get("f20")
        netflow = it.get("f62")
        out.append({
            "rank": i + 1,
            ...
            "netInflow": netflow,
            "amplitude": it.get("f7"),
            "volumeRatio": it.get("f10"),
            "amount": it.get("f6"),
            "industry": it.get("f100", ""),
            "mcap_yi": round(float(mcap) / 1e8, 2) if mcap else None,
        })
    return out
...
def daily_dragon_tiger(trade_date: str = None, min_net_buy: float = None) -> dict:
    """
    全市场龙虎榜。
    trade_date: 支持 'YYYY-MM-DD' / 'YYYYMMDD' 等格式,默认当日
...
def stock_fund_flow_120d(code: str) -> list[dict]:
    ...
    try:
        ...
        print(f"[WARN] push2 资金流请求失败: {e}")
        return []
    # d["data"] 可能是 None(不存在代码),用 (d.get("data") or {}) 兜底
...
def _em_zt_api(endpoint: str, sort: str, date: str) -> list[dict]:
    ...
    date: 支持 'YYYY-MM-DD' / 'YYYYMMDD' 等,内部统一转 YYYYMMDD(东财要求)
    返回 data.pool 原始列表（data 为 null = 非交易日 / 参数错）。"""
    # 归一化为东财要求的 YYYYMMDD 紧凑格式
    try:
        date = _normalize_date(date).replace("-", "")
    except ValueError:
        # 日期本身就错,直接返回空(上层会得到0条)
        return []
...
def em_zt_pool(date: str) -> list[dict]:
    ...
    for p in _em_zt_api("getTopicZTPool", "fbt:asc", date):
        lbc = p.get("lbc")   # 连板数(1=首板, 2=2连板...); 用 .get 防缺失
        out.append({"code": p["c"], "name": p["n"], "price": p["p"] / 1000,
            "pct": round(p["zdp"], 2), "amount": p["amount"], "float_cap": p["ltsz"],
            "turnover": round(p["hs"], 2), "limit_days": lbc, "lbc": lbc,   # lbc = limit_days 别名
...
def ths_limit_up_pool(date: str) -> list[dict]:
    """同花顺涨停揭秘（涨停原因 + 封板质量增强源）。date 支持 'YYYYMMDD' / 'YYYY-MM-DD'。
...
def full_valuation(code: str) -> dict:
    ...
    对不存在/非个股代码(指数/ETF/北交所部分票)返回带 error 字段的 dict,
    不再抛 IndexError/ValueError,便于调用方判断。
    """
    code = str(code).strip()
    # 1. 腾讯实时行情
    ...
    try:
        ...
        data = resp.read().decode("gbk")
        segs = data.split('"')
        # 腾讯无效代码返回形如 v_pk="..."; segs[1] 为空或无内容
        if len(segs) < 2 or not segs[1].strip():
            return {"code": code, "error": "无效代码或无行情数据"}
        vals = segs[1].split("~")
        if len(vals) < 50:
            return {"code": code, "error": f"行情字段不足({len(vals)}),可能非个股"}
        price = float(vals[3]) if vals[3] else 0
        mcap = float(vals[44]) if vals[44] else 0
        pe_ttm = float(vals[39]) if vals[39] else 0
        pb = float(vals[46]) if vals[46] else 0
    except Exception as e:
        return {"code": code, "error": f"行情请求失败: {type(e).__name__}: {e}"}

    # 2. 机构一致预期（直连同花顺）—— 容错:部分代码(北交所/指数)无该数据
    ...
    try:
        ...
        print(f"[WARN] full_valuation 一致预期获取失败({type(e).__name__}: {e})，仅返回行情估值")
